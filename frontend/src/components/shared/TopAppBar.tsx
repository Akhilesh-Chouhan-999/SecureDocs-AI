...
export function TopAppBar({ onMenuClick }: TopAppBarProps) {
  ...
  const isLockdown = useAppStore(state => state.isLockdown);
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 flex justify-between items-center px-margin-mobile md:px-margin-desktop h-16 bg-surface-dim/80 backdrop-blur-xl border-b border-white/10">
      ...
      <div className="flex items-center gap-md">
        <div className={`hidden md:flex items-center gap-xs px-md py-xs rounded-full border border-white/5 transition-colors ${
          isLockdown ? 'bg-error-container/20 text-error border-error/30' : 'bg-surface-container-high'
        }`}>
          <span className={`w-2 h-2 rounded-full ${isLockdown ? 'bg-error' : 'bg-green-500'} ai-pulse-dot`}></span>
          <span className="text-label-mono font-label-mono uppercase tracking-widest text-[10px]">
        ...
        <button 
          onClick={toggleTheme}
          className="w-8 h-8 flex items-center justify-center rounded-full bg-surface-container-high hover:bg-surface-variant transition-colors border border-white/5 cursor-pointer"
