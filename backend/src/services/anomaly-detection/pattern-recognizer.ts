  ...
  async detectFraudClusters(
    ...
    if (historicalDocuments && historicalDocuments.length >= 3) {
      ...
      anomalies.push({
        ...
        confidence: 0.88,
        suggestedAction: "Immediately escalate to fraud investigation team for manual review."
      });
    }
    ...
    const hasSequentialPattern = historicalDocuments.some(doc => {
      ...
      return doc.documentNumber && documentData.documentNumber &&
    ...
    if (hasSequentialPattern) {
      anomalies.push({
        ...
        severity: "high",
        description: "Document numbers indicate sequential generation typical of synthetic identity fraud.",
