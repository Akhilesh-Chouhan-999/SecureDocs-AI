...
class ReportController {
  static async generateReport(req, res, next) {
    try {
      ...
      const report = await container.get("reportService").generateFraudReport(documentId, req.user._id, anomalies || []);
  ...
  static async getReport(req, res, next) {
    try {
      const report = await container.get("reportService").getReport(req.params.reportId);
  ...
  static async getUserReports(req, res, next) {
    try {
      const result = await container.get("reportService").listReports(req.params.userId || req.user._id, req.query);
  ...
  static async downloadReport(req, res, next) {
    try {
      const download = await container.get("reportService").buildDownload(req.params.reportId);
  ...
  static async deleteReport(req, res, next) {
    try {
      const report = await container.get("reportService").deleteReport(req.params.reportId, req.user._id);
      return success(res, { message: "Report deleted successfully", reportId: report._id });
    } catch (error) { return next(error); }
  }
}

export { ReportController  };
export const default = ReportController;
