...
content = content.replace(
  /testUserId = "user-123"/g,
  'testUserId = "507f1f77bcf86cd799439011"',
);
content = content.replace(
  /testDocumentId = "doc-123"/g,
  'testDocumentId = "507f1f77bcf86cd799439012"',
);
content = content.replace(
  /testReportId = "report-123"/g,
  'testReportId = "507f1f77bcf86cd799439013"',
);
...
content = content.replace(
  /send\(\{\s*decision:\s*"approved",\s*reviewNotes:\s*"Approved after verification",\s*\}\)/g,
  'send({ decision: "approved", notes: "Approved after verification" })',
);
content = content.replace(
  /send\(\{\s*decision:\s*"manual_review",\s*reviewNotes:\s*"Further investigation needed",\s*\}\)/g,
